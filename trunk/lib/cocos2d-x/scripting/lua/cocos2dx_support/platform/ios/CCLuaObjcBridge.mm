...
int CCLuaObjcBridge::callObjcStaticMethod(lua_State *L)
{
    if (lua_gettop(L) != 3 || !lua_isstring(L, -3) || !lua_isstring(L, -2))
    {
        ...
        CCLOG("CCLuaObjcBridge invalid params");
    	return 2;
    }
    ...
    if (!className || !methodName)
    {
        lua_pushboolean(L, 0);
        lua_pushinteger(L, kCCLuaBridgeErrorInvalidParameters);
        CCLOG("CCLuaObjcBridge invalid class name or method name");
        return 2;
    }
    
    CCLOG("CCLuaObjcBridge call %s.%s", className, methodName);
    ...
    if (!targetClass)
    {
        ...
        CCLOG("CCLuaObjcBridge class %s not found", className);
        return 2;
    }
    ...
    if (methodSel == (SEL)0)
    {
        ...
        CCLOG("CCLuaObjcBridge method %s not found", methodName);
        return 2;
    }
    ...
    if (methodSig == nil)
    {
        ...
        CCLOG("CCLuaObjcBridge method signature %s error", methodName);
        return 2;
    }
