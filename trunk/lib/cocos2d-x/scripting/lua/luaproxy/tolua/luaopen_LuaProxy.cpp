...
// setUniformForSprite(CCSprite *spr, const char *uniformName, const char *type, ...)
static int tolua_LuaProxy_setUniformForSprite(lua_State *l){
#if COCOS2D_VERSION >= 0x00020100
#ifndef TOLUA_RELEASE
	tolua_Error err;
	if(!tolua_isusertype(l, 1, "CCSprite", 0, &err) || !tolua_isstring(l, 2, 0, &err) ||
		!tolua_isstring(l, 3, 0, &err)){
			tolua_error(l,"#ferror in function 'setUniformForSprite'.",&err);
			return 0;
	}
#endif
	CCSprite * o = (CCSprite *)tolua_tousertype(l, 1, NULL);
	const char * n = tolua_tostring(l, 2, NULL);
	const char * w = tolua_tostring(l, 3, NULL);
	if(o && w && n){
		float value[16];
		int valueLen;
		if(strcmp(w, "1i") == 0 || strcmp(w, "1f") == 0){
			value[0] = tolua_tonumber(l, 4, 0);
			valueLen = sizeof(float);}
		else if(strcmp(w, "2i") == 0 || strcmp(w, "2iv") == 0 || strcmp(w, "2f") == 0 || strcmp(w, "2fv") == 0){	
			if(lua_istable(l, 4)){
				for(int i = 1; i<=2; i ++){
					value[i-1] = tolua_tofieldnumber(l, 4, i, 0);
				}
			}
			else
			{
				value[0] = tolua_tonumber(l, 4, 0);
				value[1] = tolua_tonumber(l, 5, 0);
			}
			valueLen = sizeof(float)*2;}
		else if(strcmp(w, "3i") == 0 || strcmp(w, "3iv") == 0 || strcmp(w, "3f") == 0 || strcmp(w, "3fv") == 0){
			if(lua_istable(l, 4)){
				for(int i = 1; i<=3; i ++){
					value[i-1] = tolua_tofieldnumber(l, 4, i, 0);
				}
			...
			else
			{
				value[0] = tolua_tonumber(l, 4, 0);
				value[1] = tolua_tonumber(l, 5, 0);
				value[2] = tolua_tonumber(l, 6, 0);
			}
			valueLen = sizeof(float)*3;}
		else if(strcmp(w, "4i") == 0 || strcmp(w, "4iv") == 0 || strcmp(w, "4f") == 0 || strcmp(w, "4fv") == 0){	
			if(lua_istable(l, 4)){
				for(int i = 1; i<=4; i ++){
					value[i-1] = tolua_tofieldnumber(l, 4, i, 0);
				}
			...
			else
			{
				...
				value[1] = tolua_tonumber(l, 5, 0);
				value[2] = tolua_tonumber(l, 6, 0);
				value[3] = tolua_tonumber(l, 7, 0);
			}
			valueLen = sizeof(float)*4;}
			...
			valueLen = sizeof(float)*16;
		}
		else
		{
			return 1;
		}
		o->addShaderUniform(n, w, value, valueLen);
	}
#endif
	return 1;
}
...
TOLUA_API int luaopen_LuaProxy(lua_State* l){
	...
	tolua_beginmodule(l, NULL);
		tolua_function(l, "setUniformForSprite", tolua_LuaProxy_setUniformForSprite);
...
//		tolua_constant(l, "GL_TEXTURE_WIDTH", GL_TEXTURE_WIDTH);
		...
		tolua_beginmodule(l,"CCBProxy");
			...
			tolua_function(l, "getResRoot", tolua_CCBProxy_getResRoot);
			tolua_function(l, "setResRoot", tolua_CCBProxy_setResRoot);
			tolua_function(l, "getMembers", tolua_CCBProxy_getMembers);
			tolua_function(l, "changeMember", tolua_CCBProxy_changeMember);
		tolua_endmodule(l);
