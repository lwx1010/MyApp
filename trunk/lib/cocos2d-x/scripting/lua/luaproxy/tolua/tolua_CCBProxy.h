...
static int tolua_CCBProxy_getResRoot(lua_State *l){
#ifndef TOLUA_RELEASE
	tolua_Error err;
	if(!tolua_isusertable(l, 1, "CCBProxy", 0, &err)){
			tolua_error(l,"#ferror in function 'CCBProxy.getResRoot'.",&err);
			return 0;
	}
#endif
	tolua_pushstring(l, CCBProxy::getResRoot());
	return 1;
}

//CCBProxy::setResRoot(const char* pPath)
static int tolua_CCBProxy_setResRoot(lua_State *l){
#ifndef TOLUA_RELEASE
	tolua_Error err;
	if(!tolua_isusertable(l, 1, "CCBProxy", 0, &err) || !tolua_isstring(l, 2, 0, &err)){
			tolua_error(l,"#ferror in function 'CCBProxy.setResRoot'.",&err);
			return 0;
	}
#endif
	const char *pPath = tolua_tostring(l, 2, NULL);

	CCBProxy::setResRoot(pPath);
	return 0;
}

//void CCBProxy::getMembersForLua()
...
#endif
	CCBProxy *p = (CCBProxy *)tolua_tousertype(l, 1, NULL);
	p->getMembersForLua(l);

	return 1;
}

//void CCBProxy::changeMember()
static int tolua_CCBProxy_changeMember(lua_State *l){
#ifndef TOLUA_RELEASE
	tolua_Error err;
	if(!tolua_isusertype(l, 1, "CCBProxy", 0, &err) || !tolua_isstring(l, 2, 0, &err) || !tolua_isusertype(l, 3, "CCNode", 0, &err)){
		tolua_error(l,"#ferror in function 'CCBProxy.changeMember'.",&err);
		return 0;
	}
#endif
	CCBProxy *p = (CCBProxy *)tolua_tousertype(l, 1, NULL);
	const char *name = tolua_tostring(l, 2, NULL);
	CCNode *node = (CCNode *)tolua_tousertype(l, 3, NULL);

	if( p && name )
	{
		p->changeMember(name, node);
	}
	return 0;
}
