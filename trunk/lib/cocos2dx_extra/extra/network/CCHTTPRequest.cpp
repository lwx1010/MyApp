                                               ...
                                               int method)
{
    ...
    if( !request->initWithListener(listener, url, dstFile, method) )
	{
		return NULL;
	}
...
#if CC_LUA_ENGINE_ENABLED > 0
bool CCHTTPRequest::initWithListener(LUA_FUNCTION listener, const char* url, const char* dstFile, int method)
{
	m_responseFile = NULL;
	if( dstFile )
	{
		m_responseFile = fopen(dstFile, "wb");
		if( !m_responseFile )
		{
			CCLOG("CCHTTPRequest::initWithListener open dstFile failed: %s", dstFile);
			return false;
		}
	}
...
void CCHTTPRequest::update(float dt)
{
	...
	{
		//CCLOG("downloading... %d%%", m_total?(int)(m_now/m_total*100):0);
		...
		if (m_listener)
		{
			...
			dict["total"] = CCLuaValue::floatValue(m_total);
			dict["now"] = CCLuaValue::floatValue(m_now);
    ...
    if (m_curlState != kCCHTTPRequestStateIdle )
    {
...
size_t CCHTTPRequest::onWriteData(void* buffer, size_t bytes)
{
	if( m_responseFile )
	{
		fwrite(buffer, bytes, 1, m_responseFile);
		return bytes;
	}
