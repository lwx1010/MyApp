...
string md5(const string& input)
{
	...
	CCCrypto::MD5((void*)input.c_str(), input.length(), buffer);

	char* hex = CCCrypto::bin2hex(buffer, CCCrypto::MD5_BUFFER_LENGTH);
	string str = hex;
	delete[] hex;
...
void CCCrypto::MD5(void* input, int inputLength, unsigned char* output)
{
    MD5_CTX ctx;
    X6MD5_Init(&ctx);
    X6MD5_Update(&ctx, input, inputLength);
    X6MD5_Final(output, &ctx);
}
