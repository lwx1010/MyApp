...
function StateMachine:resetState()
	self.current_ = self.initial_.state
	return self.initial_.state
end
...
function StateMachine:doEvent(name, param)
...
--    if args then
--    	printf("Attacker is 向下箭头")
--    	printf(args.attackTable[1])
--    end
--    if from == to then
--		dump(event)
--		assert(false)
--	end
	printf("MESSAGE: event %s, from %s to %s", event.name, event.from, event.to)
