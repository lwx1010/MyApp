...
int LuaHostWin32::run(void)
{
    ...
    do
    {
        ...
        // set environments
		const string& strProj = m_project.getProjectDir();
		if( strProj.length()>0 )
		{
			SetCurrentDirectoryA(strProj.c_str());
			CCFileUtils::sharedFileUtils()->setSearchRootPath(strProj.c_str());
		}
        ...
        // cleanup
		cocos2d::extra::SwfAnimCache::sharedSwfAnimCache()->purgeSharedSwfAnimCache();
